...
import facefusion.processors.frame.core2 as frame_processors
from facefusion import wording
from facefusion.core2 import update_status
...
def process_frame23(source_face : Face, reference_face : Face, temp_frame : Frame) -> Frame:
	if 'reference' in facefusion.globals.face_recognition:
		
		# print('reference:',type(reference_face))
		if reference_face:
			temp_frame=swap_face(source_face, reference_face, temp_frame)
...
def process_frames23(source_path : str, temp_frame_paths : List[str], update_progress : Update_Process) -> None:
	source_face = get_one_face(read_static_image(source_path))
	# reference_face = get_face_reference() if 'reference' in facefusion.globals.face_recognition else None
	for temp_frame_path in temp_frame_paths:

		# print('temp_frame_path:',temp_frame_path)

		# temp_frame = read_static_image(temp_frame_path)
		# # temp_frame = read_image(temp_frame_path)
		# reference_face = get_one_face(temp_frame, facefusion.globals.reference_face_position) if 'reference' in facefusion.globals.face_recognition else None
		# result_frame = process_frame(source_face, reference_face, temp_frame)
		...
		target_frame = read_static_image(temp_frame_path)
		reference_face = get_one_face(target_frame, facefusion.globals.reference_face_position) if 'reference' in facefusion.globals.face_recognition else None
		result_frame = process_frame23(source_face, reference_face, target_frame)
		write_image(temp_frame_path, result_frame)
...
def process_video23(source_path : str, temp_frame_paths : List[str]) -> None:
	# conditional_set_face_reference(temp_frame_paths)
	frame_processors.multi_process_frames(source_path, temp_frame_paths, process_frames23)
