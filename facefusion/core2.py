...
from facefusion.processors.frame.core2 import get_frame_processors_modules, load_frame_processor_module
...
def cli() -> None:
	...
	# frame processors
	available_frame_processors = list_module_names('facefusion/processors/frame/modules2')
...
def apply_args(program) -> None:

	# args = argparse.Namespace(**program)
	...
	# frame processors
	# available_frame_processors = list_module_names('facefusion/processors/frame/modules2') 
	available_frame_processors = list_module_names('facefusion23/facefusion/processors/frame/modules2') #sdapi加目录
...
def conditional_process() -> None:
	for frame_processor_module in get_frame_processors_modules(facefusion.globals.frame_processors): #获取处理模块
...
def process_image() -> None:
	if predict_image(facefusion.globals.target_path): #opennsfw2对图片/视频进行鉴黄识别
		return
	shutil.copy2(facefusion.globals.target_path, facefusion.globals.output_path) #复制文件
	# process frame
	for frame_processor_module in get_frame_processors_modules(facefusion.globals.frame_processors):
...
def process_video() -> None:
	...
	create_temp(facefusion.globals.target_path) #创建临时帧目录
	# extract frames
	update_status(wording.get('extracting_frames_fps').format(fps = fps))  
	extract_frames(facefusion.globals.target_path, fps) #将视频切成帧


	# process frame
	temp_frame_paths = get_temp_frame_paths(facefusion.globals.target_path)  #获取所有帧文件名
	if temp_frame_paths: #处理所有帧
		for frame_processor_module in get_frame_processors_modules(facefusion.globals.frame_processors):
			update_status(wording.get('processing'), frame_processor_module.NAME)
...
def process_video23() -> None:
	# process frame
	# temp_frame_paths = get_temp_frame_paths(facefusion.globals.target_path)  #获取所有帧文件名

	temp_frame_paths=get_file(facefusion.globals.target_path)

	print('所有图片路径：',temp_frame_paths)

	if temp_frame_paths: #处理所有帧
		for frame_processor_module in get_frame_processors_modules(facefusion.globals.frame_processors):
			update_status(wording.get('processing'), frame_processor_module.NAME)
			frame_processor_module.process_video23(facefusion.globals.source_path, temp_frame_paths)
			frame_processor_module.post_process()
	else:
		update_status(wording.get('temp_frames_not_found'))
		return
